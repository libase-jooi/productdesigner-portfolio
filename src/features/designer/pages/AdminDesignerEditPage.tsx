...
function BasicInfoForm({ designer: d }: { designer: DesignerWithRelations }) {
  return (
    <div className="rounded-2xl bg-surface-container-low p-8 space-y-8">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <FormField label="ステータス" required>
          <NativeSelect defaultValue={d.status} options={[...DesignerStatus]} />
        </FormField>

        <FormField label="ソースURL" required className="sm:col-span-2">
          <Input
        ...
        <FormField label="ソースタイプ" required>
          <NativeSelect defaultValue={d.sourceType} options={[...SourceType]} />
        </FormField>

        <FormField label="取り込み日">
          <Input
            defaultValue={new Date(d.importedAt).toLocaleDateString("ja-JP")}
            disabled
            className="bg-surface-container border-none rounded-lg h-10 type-body-md text-on-surface-variant"
          />
        </FormField>
        ...
        <FormField label="プロフィール画像URL" className="sm:col-span-2">
          <div className="flex gap-3">
            <Input
              defaultValue={d.profileImageUrl ?? ""}
              placeholder="https://..."
...
function ValidationSummary({
      ...
      <div className="rounded-2xl bg-surface-container-low p-8 text-center">
        <div className="space-y-2">
          ...
          <p className="type-body-sm text-on-surface-variant">
            まだデータがありません。AI取り込みを実行するか、手動で情報を入力してください。
          </p>
        </div>
      </div>
    );
  ...
  const hasIssue = allItems.filter(
    (i) => i.review === "要修正" || i.confidence === "低"
  ).length;

  const confidenceRate = Math.round((highCount / total) * 100);
  const reviewRate = Math.round((confirmedCount / total) * 100);
  ...
  return (
    <div className="rounded-2xl bg-surface-container-low p-6 space-y-5">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <MiniStat label="AI信頼度 高" value={`${confidenceRate}%`} progress={confidenceRate} />
        <MiniStat label="確認済み" value={`${reviewRate}%`} progress={reviewRate} />
        <MiniStat
          label="未確認"
...
function ReviewBadge({ status }: { status: ReviewStatus }) {
  ...
  return (
    <Badge className={`rounded-full border-none type-label-sm ${variants[status]}`}>
      {status}
    </Badge>
  );
}
